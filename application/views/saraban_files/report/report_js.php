			...
			if(date2 != ""){
					if(date1 == ""){
						...
						chk         = true;
                                       ...
                                       var api_link = "<?php //echo base_url(); ?>Saraban/get_report"; 
...
+'<td>'+info['previous'][i].topic+'</td>'   																+'<td>'+formatDate(new Date(info['previous'][i].date_add))+'</td>'
+'</tr>';
                                            		                                      	}
    datail = '<table class="table table-bordered datatable table-striped" id="table-1">'+tabheard+datail+'</tbody>'+'</table>';
		...
		function dosearch(){ 
			...
			if((dayend != "")&&(monthend != "")&&(yearend != "")){
                    ...
                    $.post('<?php echo base_url('Saraban/get_report')?>' , { id : id_user, id_saraban : id_saraban, topic : topic, date1 : date1, date2 : date2, in_out : in_out }, function(data){

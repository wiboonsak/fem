	...
	<?php function DateThai($strDate){
		  ...
		  foreach($documentData->result() as $documentData2){ }
            ...
            $yearoffice2 = $this->Allowance_model->get_month($documentData2->date_office2,'4');
                  ...
                  if(($documentData2->date_thailand != '') && ($documentData2->date_thailand != '0000-00-00')){
            $datethailand = $this->Allowance_model->get_month($documentData2->date_thailand,'3');
            $monthailand = $this->Allowance_model->get_month($documentData2->date_thailand,'1');
            $yearthailand = $this->Allowance_model->get_month($documentData2->date_thailand,'4');
                              }else{
                                $datethailand = '';
                                $monthailand = '';
                                $yearthailand = '';
                              }
                  if(($documentData2->date_thailand2 != '') && ($documentData2->date_thailand2 != '0000-00-00')){
            $datethailand2 = $this->Allowance_model->get_month($documentData2->date_thailand2,'3');
            $monthailand2 = $this->Allowance_model->get_month($documentData2->date_thailand2,'1');
            $yearthailand2 = $this->Allowance_model->get_month($documentData2->date_thailand2,'4');
                              }else{
                                $datethailand2 = '';
                                $monthailand2 = '';
                                $yearthailand2 = '';
...
if(($documentData2->withdraw == '1') && ($documentData2->saraban_number != '')){
	  ...
	  } else if(($this->session->userdata['logged_in']['status'] == 'user') && ($this->session->userdata['logged_in']['statusApprove'] == 'Yes') && ($documentData2->check_doc == '1') && ($documentData2->saraban_number != '') && ($documentData2->approve_status2 == '2')){
	  ...
	  } else if(($this->session->userdata['logged_in']['status'] == 'user') && ($this->session->userdata['logged_in']['statusApprove'] == 'Yes') && ($documentData2->check_doc2 == '1') && ($documentData2->saraban_number!= '')){
	         
	      $saraban_number = $this->Saraban_model->explode_sarabanNumber($documentData2->saraban_number,'1');
	  
	  } else if(($this->session->userdata['logged_in']['status'] == 'user') && ($documentData2->check_doc2 == '0') && ($documentData2->check_doc == '1') && ($documentData2->approve_status == '1') && ($documentData2->saraban_number != '')){ 
    ...
    <tr>
      <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ด้วย <?php echo $userDetail2->titlename.$userDetail2->firstname.' '.$userDetail2->lastname; ?><!--  วรดร ไผ่เรือง--> <?php echo $careerData2->career; ?>&nbsp;ตำแหน่ง<?php echo $positionData2->position; ?>&nbsp;<?php //if($career_id == '5'){?>ตำแหน่งเลขที่ <?php echo $userDetail2->position_number;  //}?>&nbsp;<!-- { เฉพาะข้าราชการมี&nbsp; --><?php if(in_array($userDetail2->career_id, $array2)){?>อัตราค่าจ้าง <?php echo number_format($userDetail2->wage); ?> บาท<?php } ?>&nbsp;สังกัดคณะการจัดการสิ่งแวดล้อม มีความประสงค์จะเดินทางไป<!--ปฎิบัติงานเพื่อ--> <select name="travel_for" id="travel_for" class="input-data"><option value="">-- เลือก --</option><option value="1" <?php if($documentData2->travel_for == '1'){ echo 'selected'; } ?> >เข้าร่วมประชุม</option><option value="2" <?php if($documentData2->travel_for == '2'){ echo 'selected'; } ?> >เข้าร่วมประชุมทางวิชาการ</option><option value="3" <?php if($documentData2->travel_for == '3'){ echo 'selected'; } ?> >ฝึกอบรม</option><option value="4" <?php if($documentData2->travel_for == '4'){ echo 'selected'; } ?> >ดูงาน</option><option value="5" <?php if($documentData2->travel_for == '5'){ echo 'selected'; } ?> >ประชุมเชิงปฎิบัติการ</option><option value="6" <?php if($documentData2->travel_for == '6'){ echo 'selected'; } ?> >ปฏิบัติงานเพื่อปรึกษาหารือ</option></select>&nbsp;เรื่อง <input type="text" name="subject_form" id="subject_form" placeholder="ระบุชื่อเรื่อง" class="input-data" <?php if(in_array($userDetail2->career_id, $array1)){ ?>style="width: 100%;"<?php } ?> <?php if(in_array($userDetail2->career_id, $array2)){ ?>style="width: 79%;"<?php } ?> value="<?php if($documentData2->subject_form != ''){ echo $documentData2->subject_form; }?>">&nbsp;<br>ณ&nbsp;<input type="text" name="place" id="place" placeholder="ระบุสถานที่" class="input-data" style="width: 720px;" value="<?php if($documentData2->place != ''){ echo $documentData2->place; }?>">&nbsp;มีกำหนด&nbsp;<span id="numDay"><?php echo $this->Allowance_model->DateDiff2($documentData2->date_start , $documentData2->date_end);?>       </span>&nbsp;วัน&nbsp;ตั้งแต่วันที่&nbsp;<select class="input-data" id="daystart" name="daystart" >
						...
						$yearthai = $y+543
							...
							<?php }	?>
							</select>&nbsp;โดยใช้เงินสนับสนุนจาก&nbsp;<input type="text" name="money_source" id="money_source" placeholder="ระบุแหล่งเงิน" class="input-data" style="width: 75%;" value="<?php if($documentData2->money_source != ''){ echo $documentData2->money_source; }?>">	  
	<?php if(($documentData2->reason_request == '1') && ($documentData2->withdraw == '1')){?>&nbsp;<U>ตามสิทธิคงเหลือ <?php echo number_format($balance);?> บาท</U><?php } ?>&nbsp;ดังเอกสารที่แนบมาพร้อมนี้</td>
    </tr>
...
<?php if($money == '1'){
	  ...
	  $checkURI2 = $this->uri->segment(6);
	
	  if(($documentData2->approve_id == $this->session->userdata['logged_in']['id']) && ($checkURI2 != '1')){
			   
			$setApprove = 0;
			   
	  } else {
			   
			$setApprove = $documentData2->approve_id;
	  }	
	  
	  if(($this->session->userdata['logged_in']['status'] != 'admin_saraban') && ($setApprove != $this->session->userdata['logged_in']['id']) && ($checkURI2 != '1')){ ?>	  
    <tr>
      <td height="40">&nbsp;</td>
    </tr>
	<tr>
	...
	<?php   $withdrawNum = $withdrawNum->num_rows();
			//$num_withdrawData = $withdrawData->num_rows();
			if($withdrawNum > 0){
	...
	?>	  
        ...
        <table border="0" cellpadding="0" cellspacing="0">
          <tbody>
			...
			<?php if($withdrawData_out2->passage_baht != '0.00'){ ?>   
            <tr>
              <td align="right"><?php echo $num_withdraw?>.</td>
      ...
      <td height="40"><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>(ประมาณการค่าใช้จ่ายในประเทศ)</strong><br><br>
        <table border="0" cellpadding="0" cellspacing="0">
          <tbody>
            ...
            <tr>
              <td width="10%" align="right"><?php echo $num_withdraw?>.</td>
              <td width="12%" align="left">&nbsp;ค่าที่พัก</td>
			...
			<?php if($withdrawData2->passage_baht != '0.00'){ ?>   
            <tr>
              <td align="right"><?php echo $num_withdraw?>.</td>
              <td align="left">&nbsp;ค่าพาหนะ</td>
              <td align="center">&nbsp;</td>
		...
		<?php if(($num_withdrawData > 0) && ($num_withdrawDataOut == 0)){ ?>	   
		...
		<?php if(($num_withdrawData > 0) && ($num_withdrawDataOut > 0)){ 
			  	...
			  	$total_price = $withdrawData2->total_price + $withdrawData_out2->total_price;
		?>	   
			...
			<tr height="40">
              ...
              <td colspan="4" align="left">&nbsp;</td>
              <td align="right">&nbsp;&nbsp; <strong>จำนวนเงินรวมทั้งสิ้น </strong>&nbsp;</td>
              <td align="right"><span id="txt_amount"><strong><?php echo number_format($total_price)?></strong></span></td>
              <td align="left">&nbsp;<strong>บาท</strong></td>
            </tr>  
		<?php } ?>  
          </tbody>
		  ...
		  <div id="div_vacation">
				...
				<div class="col-sm-3" style="padding-left: 0px;padding-right: 0px;">
							...
							<?php for($a=1; $a<=31; $a++){ 
									...
									if($a < 10){  $txt = '0';  } else { $txt =''; }
							?>								
                                                                <option value="<?php echo $txt.$a?>" <?php if(($documentData2->date_thailand != '')&&($datethailand==$txt.$a)){ echo 'selected'; }?>><?php echo $a?></option>	
							<?php }	?>
							</select>
							...
							?>								
								<option value="<?php echo $txt.$x?>" <?php if(($documentData2->date_thailand != '')&&($monthailand==$txt.$x)){ echo 'selected'; }?>><?php echo $month?> </option>
						...
						$yearthai = $y+543
							?>								
								<option value="<?php echo $y?>" <?php if(($documentData2->date_thailand != '')&&($yearthailand==$y)){ echo 'selected'; }?>><?php echo $yearthai?></option>
				...
				<div class="col-sm-3" style="padding-left: 0px;padding-right: 0px;">
							...
							<?php for($a=1; $a<=31; $a++){ 
									...
									if($a < 10){  $txt = '0';  } else { $txt =''; }
							?>								
                                                                <option value="<?php echo $txt.$a?>" <?php if(($documentData2->date_thailand2 != '')&&($datethailand2==$txt.$a)){ echo 'selected'; }?>><?php echo $a?></option>	
							<?php }	?>
							</select>
							...
							?>								
								<option value="<?php echo $txt.$x?>" <?php if(($documentData2->date_thailand2 != '')&&($monthailand2==$txt.$x)){ echo 'selected'; }?>><?php echo $month?> </option>
						...
						$yearthai = $y+543
							?>								
								<option value="<?php echo $y?>"  <?php if(($documentData2->date_thailand2 != '')&&($yearthailand2==$y)){ echo 'selected'; }?>><?php echo $yearthai?></option>
...
<!---------------------------------------------------------------------------->
					...
					<?php 	if(($this->session->userdata['logged_in']['status'] == 'user') && ($documentData2->check_doc2 == '3') && ($documentData2->approve_id != $this->session->userdata['logged_in']['id']) && ($documentData2->approve_id2 != $this->session->userdata['logged_in']['id']) && (($documentData2->check_doc == '3') || ($documentData2->check_doc == '0'))){ ?>		
							<div class="form-group">
							<label class="col-sm-12 control-label" style="font-size: 10pt; color: red; line-height: 24px; text-align: left; padding-bottom: 15px;"><strong>111ข้อแนะนำการแนบไฟล์</strong><br>
					...
					<?php 	if(($this->session->userdata['logged_in']['status'] == 'user') && ($documentData2->check_doc == '1') && ($documentData2->approve_status == '1') && ($documentData2->approve_id != $this->session->userdata['logged_in']['id']) && ($documentData2->approve_id2 != $this->session->userdata['logged_in']['id']) && (($documentData2->check_doc2 == '3') || ($documentData2->check_doc2 == '0'))){ ?>
			
							<div class="form-group">
							<label class="col-sm-12 control-label" style="font-size: 10pt; color: red; line-height: 24px; text-align: left; padding-bottom: 15px;"><strong>222ข้อแนะนำการแนบไฟล์</strong><br>
							1. การอัพโหลดไฟล์ ทุกช่องสามารถอัพโหลดได้ครั้งละ 1 ไฟล์เท่านั้น และหากมีหลายหน้าหรือหลายภาพ ควรจัดทำและบันทึกเป็นไฟล์ .pdf เพื่อสะดวกต่อการอัพโหลดไฟล์และเรียกดูการแสดงผล<br>
							2. รองรับไฟล์นามสกุล .pdf , .jpg , .png , .gif , .docx เท่านั้น<br> 
							3. ขนาดไฟล์ไม่ควรเกิน 2MB<br> 
							4. กรุณากดปุ่ม Upload ทุกช่องที่มีการแนบไฟล์</label>
					...
					<?php	if(($documentData2->check_doc != '3') && ($documentData2->check_doc != '0') && ($documentData2->check_doc2 == '3') && (($this->session->userdata['logged_in']['status'] == 'user'))){ 
							
								$showDiv = 'style="display: block"';
								$btnPDF = 'style="display: inline-block"';
	
							} else if(($documentData2->check_doc2 != '3') && ($documentData2->check_doc2 != '0') && ($documentData2->check_doc == '1') && (($this->session->userdata['logged_in']['status'] == 'user'))){ 
							...
							} else if(($documentData2->check_doc != '3') && ($this->session->userdata['logged_in']['status'] == 'admin_saraban')){ 
							
								$showDiv = 'style="display: block"';
								$btnPDF = 'style="display: inline-block"';

							} else if(($documentData2->check_doc != '3') && ($this->session->userdata['logged_in']['status'] == 'admin')){ 
					...
					?>							
							...
							<div class="form-group" <?php echo $showDiv?> >
								<label class="col-sm-3 control-label">แนบไฟล์หนังสือเชิญ</label>
								<div class="col-sm-9">
									<div class="row">										
					...
					<?php if((($this->session->userdata['logged_in']['status'] == 'admin_saraban') || ($this->session->userdata['logged_in']['status'] == 'admin')) && (($documentData2->check_doc != '0') && ($documentData2->check_doc2 != '0'))){ 
					...
					?>		
								<div class="form-group" <?php if($this->session->userdata['logged_in']['statusApprove'] == 'Yes'){?>style="display: none"<?php } ?> >
						...
						<?php /*if(($documentData2->approve_id == $this->session->userdata['logged_in']['id']) || (($documentData2->approve_id2 == $this->session->userdata['logged_in']['id']))){
							$checkURI2 = '2';
						}*/ ?>							
													
						<?php if((($this->session->userdata['logged_in']['status'] == 'admin') || ($documentData2->approve_id2 == $this->session->userdata['logged_in']['id'])) && ($documentData2->check_doc == '1') && ($documentData2->check_doc2 != '3') && ($documentData2->check_doc2 != '0') && ($checkURI2 != '1') && ($this->session->userdata['logged_in']['status'] != 'admin_saraban')){ ?>
								<div class="form-group">
									<label for="command" class="col-sm-3 control-label">เสนอ</label>
									<div class="col-sm-9">									
										<input type="text" class="form-control" id="command" style="font-family:'THSarabunPSK-Bold' !important; color: #000000; font-size: 14pt; border: 1px solid #A4A4A4; border-radius: 4px; padding: 2px; margin: 2px;" value="<?php if($documentData2->offer_text2 != ''){ echo $documentData2->offer_text2; }?>" <?php if(($this->session->userdata['logged_in']['status'] == 'user') && ($this->session->userdata['logged_in']['statusApprove'] == 'Yes')){ echo 'readonly'; }?> >
									</div>
								</div>
						<?php } ?>							
							
						<?php if((($this->session->userdata['logged_in']['status'] == 'admin_saraban') || ($documentData2->approve_id == $this->session->userdata['logged_in']['id'])) && ($documentData2->check_doc != '3') && ($documentData2->check_doc != '0') && ($checkURI2 != '2') && ($this->session->userdata['logged_in']['status'] != 'admin')){ ?>
								<div class="form-group">
									<label for="command" class="col-sm-3 control-label">เสนอ</label>
									<div class="col-sm-9">									
										<input type="text" class="form-control" id="command" style="font-family:'THSarabunPSK-Bold' !important; color: #000000; font-size: 14pt; border: 1px solid #A4A4A4; border-radius: 4px; padding: 2px; margin: 2px;" value="<?php if($documentData2->offer_text != ''){ echo $documentData2->offer_text; }?>" <?php if(($this->session->userdata['logged_in']['status'] == 'user') && ($this->session->userdata['logged_in']['statusApprove'] == 'Yes')){ echo 'readonly'; }?> >
									</div>
								</div>
                		<?php } ?>								
							...
							<div class="form-group">
								...
								<?php if(($this->session->userdata['logged_in']['status'] == 'user') && ($documentData2->approve_id != $this->session->userdata['logged_in']['id']) && ($documentData2->approve_id2 != $this->session->userdata['logged_in']['id']) && (($documentData2->check_doc == '3') || ($documentData2->check_doc == '0')) && ($documentData2->check_doc2 == '3')){ ?><!-- && ($documentData2->approve_status2 == '')-->
								 		...
								 		<?php //if(($documentData2->check_doc == '0') || ($documentData2->check_doc == '3')){ ?>
								...
								<?php if(($this->session->userdata['logged_in']['status'] == 'user') && ($documentData2->approve_id != $this->session->userdata['logged_in']['id']) && ($documentData2->approve_id2 != $this->session->userdata['logged_in']['id']) && (($documentData2->check_doc2 == '3') || ($documentData2->check_doc2 == '0')) && ($documentData2->check_doc == '1') && ($documentData2->approve_status != '2')){ ?>
										<!-- && ($documentData2->approve_status == '')-->
								 		<button type="button" class="btn btn-orange btn-icon" onclick="check_dateVacation()" >บันทึก<i class="entypo-search"></i></button>
								 		 
								 		<?php if($documentData2->check_doc2 == '0'){ ?>
								 		<?php //if(($documentData2->check_doc2 == '0') || ($documentData2->check_doc2 == '3')){ ?>
								...
								<?php if(($this->session->userdata['logged_in']['statusApprove'] == 'Yes') && (($documentData2->approve_id == $this->session->userdata['logged_in']['id']) || ($documentData2->approve_id2 == $this->session->userdata['logged_in']['id']))){
										$btnPDF = 'style="display: none;"';
								} ?>										
										<button type="button" <?php echo $btnPDF?> class="btn btn-blue btn-icon" onclick="Preview('<?php echo $url_preview?>','<?php echo $documentData2->id?>','<?php echo $documentData2->user_create?>')">ดูตัวอย่างบน PDF<i class="entypo-search"></i></button>
								<?php //} ?>									
									</p>
									
									<?php if(($this->session->userdata['logged_in']['status'] == 'user') && ($documentData2->approve_id != $this->session->userdata['logged_in']['id']) && ($documentData2->approve_id2 != $this->session->userdata['logged_in']['id']) && (($documentData2->check_doc == '3') || ($documentData2->check_doc == '0')) && ($documentData2->check_doc2 == '3')){ ?>
					...
					<?php if((($documentData2->approve_id == $this->session->userdata['logged_in']['id']) || ($documentData2->approve_id2 == $this->session->userdata['logged_in']['id'])) && ($this->session->userdata['logged_in']['statusApprove'] == 'Yes')){ ?>		
								...
								<div class="col-sm-9">	
									<span id='alert_error'></span>							 
								 	<p class="bs-example"> 
								 		...
								 		<!--<button type="button" class="btn btn-red btn-icon" onclick="dusubmit_approve('<?php //echo $documentData2->id?>','<?php //echo $documentData2->approve_status?>','0')">ไม่อนุมัติ<i class="entypo-mail" ></i> </button>-->
									...
									</p><!--onclick="dusubmit_approve('<?php //echo $documentData2->id?>','<?php //echo $documentData2->approve_status?>','0')"-->
								</div>
							</div> 
					<?php } ?>			
							...
							<input type="hidden" id="num_withdrawDataOut" name="for_type" value="<?php echo $num_withdrawDataOut;?>">					

...
<?php if(($documentData2->check_doc != '3') && ($documentData2->check_doc != '0') && ($documentData2->check_doc2 == '3') && ($checkURI != 'create_outbound') && ($checkURI != 'outbound_withdraw')){ ?>
<script>
...
<?php } else if(($documentData2->check_doc2 != '3') && ($documentData2->check_doc2 != '0') && ($documentData2->check_doc == '1') && ($checkURI != 'create_outbound') && ($checkURI != 'outbound_withdraw')){ ?>
<script>
		$(document).ready(function(){
			$('.input-data').attr('readonly', true);
			$('select.input-data').attr('disabled', true); 
  ...
  $('#textchange').val('1');
	...
	function saveData(){
		...
		if(file1 ==''){				
			...
			if(document_id == ''){
				...
				var fnName = 'save_outboundWithdraw';
			   
			} else {
				   
				var fnName = 'edit_outboundWithdraw/'+document_id;				   
			}
			...
			$.ajax({
				...
				success:function(data){
					...
					if(id != 'x'){
						...
						if(document_id == ''){						
							window.location.href ='<?php echo base_url();?>Allowance2/detailOutbound/'+withdraw+'/'+for_type+'/'+id;
						}							
					}					
			    }
            })		
	...
	function check_dateVacation(){
		...
		if($('#CHvacation').val() == '1'){
			...
			if(($('#dayoffice').val() == '')||($('#monthoffice').val() == '')||($('#yearoffice').val() == '')){
				...
				alert('กรุณาระบุวันที่เดินทางกลับถึงสำนักงาน !');
				return false;
			} else if(($('#daythailand').val() == '')||($('#monththailand').val() == '')||($('#yearthailand').val() == '')){
				
				alert('กรุณาระบุวันที่เดินทางออกจากประเทศไทย !');
				return false;
			} else if(($('#daythailand2').val() == '')||($('#monththailand2').val() == '')||($('#yearthailand2').val() == '')){
				
				alert('กรุณาระบุวันที่เดินทางกลับถึงประเทศไทย !');
				return false;
	...
	function submit_andSend(withdrawNum, num_withdrawDataOut, num_withdrawData, dataid){
		...
		$.post('<?php echo base_url('Allowance2/do_submit_andSend')?>' , { document_id : document_id , subject_document : subject_document , withdraw : withdraw , withdrawNum : withdrawNum , num_withdrawDataOut : num_withdrawDataOut , num_withdrawData : num_withdrawData } , function(data){
			...
			if(data == 1){
				 $.post('<?php echo base_url('document_sendmail/send_mail')?>' , { dataid : dataid, edit : edit  } , function(data2){
						swal({
							title: "สำเร็จ",
    ...
    function sendpass(dataid,p_or_f,textpass){		
		...
		if((sendto == "") && (p_or_f == 'pass')){
			...
			function(){  //console.log('1p_or_f.....'+p_or_f);
	...
	function dusubmit_approve(id_allownace,chk_approve,approve_status){						
				...
				swal({
					...
					$.post('<?php echo base_url('allowance/approve_allowance')?>' , { id_allowance : id_allownace , approve_status : approve_status , chk_approve : chk_approve , notapproved : notapproved } , function(data){
						...
						if(data != 0){
                            if(withdraw == '0'){
                                $.post('<?php echo base_url('document_sendmail/send_mailapprove0')?>' , { id_allownace : id_allownace , approve_status : approve_status} , function(data2){
                                               //console.log('.....'+data2);                 
									swal({
                               ...
                               $.post('<?php echo base_url('document_sendmail/send_mailapprove1')?>' , { id_allownace : id_allownace , approve_status : approve_status} , function(data2){   
                                          //console.log('.....'+data2);                
									swal({
									  title: "ยืนยันการอนุมัติสำเร็จ",
									  //text: "ยืนยันข้อมูล และส่งข้อมูลสำเร็จ",

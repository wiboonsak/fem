<link type="text/css" rel="stylesheet" href="<?php echo base_url();?>allowance/assets/FontTHSarabun/styles.css">
	<div class="main-content">			
		...
		<h2 style="text-align: center; padding-top: 20px;">สร้างคำขออนุมัติเดินทางในประเทศ</h2>
		<br/><br/>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tbody>
    <tr>
      <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ด้วย <?php echo $userDetail2->titlename.$userDetail2->firstname.' '.$userDetail2->lastname; ?><!--  วรดร ไผ่เรือง--> <?php echo $careerData2->career; ?>&nbsp;ตำแหน่ง<?php echo $positionData2->position; ?>&nbsp;<?php //if($career_id == '5'){?>ตำแหน่งเลขที่ <?php echo $userDetail2->position_number;  //}?>&nbsp;<!-- { เฉพาะข้าราชการมี&nbsp; --><?php if(in_array($userDetail2->career_id, $array2)){?>อัตราค่าจ้าง <?php echo number_format($userDetail2->wage); ?> บาท<?php } ?>&nbsp;สังกัดคณะการจัดการสิ่งแวดล้อม มีความประสงค์จะเดินทางไป<!--ปฎิบัติงานเพื่อ--> <select name="travel_for" id="travel_for" class="input-data"><option value="">-- เลือก --</option><option value="1">เข้าร่วมประชุม</option><option value="2">เข้าร่วมประชุมทางวิชาการ</option><option value="3">ฝึกอบรม</option><option value="4">ดูงาน</option><option value="5">ประชุมเชิงปฎิบัติการ</option><option value="6">ปฏิบัติงานเพื่อปรึกษาหารือ</option></select>&nbsp;เรื่อง <input type="text" name="subject_form" id="subject_form" placeholder="ระบุชื่อเรื่อง" class="input-data" <?php if(in_array($userDetail2->career_id, $array1)){ ?>style="width: 100%;"<?php } ?> <?php if(in_array($userDetail2->career_id, $array2)){ ?>style="width: 79%;"<?php } ?> >&nbsp;<br>ณ&nbsp;<input type="text" name="place" id="place" placeholder="ระบุสถานที่" class="input-data" style="width: 720px;">&nbsp;มีกำหนด&nbsp;<span id="numDay">0</span>&nbsp;วัน&nbsp;ตั้งแต่วันที่&nbsp;<select class="input-data" id="daystart" name="daystart" >
						...
						$yearthai = $y+543
							...
							<?php }	?>
							</select>ถึง&nbsp;<select class="input-data" id="dayend" name="dayend" onChange="countDays()">
                               <option value="00">วัน</option>
							<?php for($a=1; $a<=31; $a++){ 
							...
							?>								
                                                                <option value="<?php echo $txt.$a?>"><?php echo $a?></option>	
							<?php }	?>
							</select><select class="input-data" id="monthend" name="monthend" onChange="countDays()">
                               <option value="00">เดือน</option>
							<?php for($x=1; $x<=12; $x++){ 
							...
							<?php }	?>
							</select><select class="input-data" id="yearend" name="yearend" onChange="countDays()">
                               <option value="00">ปี</option>
							<?php for($y=2017; $y<=2032; $y++){ 
						$yearthai = $y+543
							?>								
							...
							<?php }	?>
							</select>&nbsp;โดยใช้เงินสนับสนุนจาก&nbsp;<input type="text" name="money_source" id="money_source" placeholder="ระบุแหล่งเงิน" class="input-data" style="width: 75%;"><?php if(($money == '1') && ($reason_request == '1')){?>&nbsp;<U>ตามสิทธิคงเหลือ <?php echo number_format($balance);?> บาท</U><?php } ?>&nbsp;ดังเอกสารที่แนบมาพร้อมนี้</td>
    </tr>
...
<?php } ?>
	...
	<tr>
      <td>
		  <div class="checkbox" style="margin-left: 7%;"><label><input type="checkbox" id="CHvacation" name="CHvacation" onClick="check_vacation2(this.checked,'a')" value="0" >ต้องการลาพักผ่อน</label></div>
	  </td>
    </tr>
		  ...
		  <div id="div_vacation">
				...
				<div class="col-sm-3" style="padding-left: 0px;padding-right: 0px;">
			  	...
			  	<div class="form-group">
					...
					<div class="col-sm-5">
						<input type="text" class="form-control input-data" id="date_vacation" name="date_vacation" placeholder="ตัวอย่าง 10 เมษายน 2562 , 15 เมษายน 2562 หรือ 10-12 เมษายน 2562">					
					</div>
				</div>		  
		  </div>
	  </td>
	...
	<tr>
      <td height="40">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;จึงเรียนมาเพื่อโปรดพิจารณาด้วย จะเป็นพระคุณยิ่ง
	  </td>
    </tr>
	<tr>
      <td>&nbsp;</td>
	...
	<tr>
      <td height="40">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;จึงเรียนมาเพื่อโปรดพิจารณา หากเห็นชอบด้วยกรุณาอนุมัติ และ
        <table border="0" cellpadding="0" cellspacing="0">
          <tbody>
            <tr>
              <td width="85" align="right"><input type="checkbox" name="select_checkbox[]" id="select_checkbox" value="1"></td>
              <td width="1101" align="left">&nbsp; แจ้งกองคลัง, งานทะเบียนประวัติ และแจ้งวันเดินทางทีแน่นอนต่อไปด้วย</td>
            </tr>
            <tr>
              <td width="85" align="right"><input type="checkbox" name="select_checkbox[]" id="select_checkbox2" value="2"></td>
	...
	<tr>
      <td height="40">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="select_checkbox[]" id="select_checkbox" value="3" onClick="showSpan(this.checked)"> &nbsp;&nbsp;หนังสือเดินทางมีอายุไม่ถึง 6 เดือน ณ วันเดินทาง</td>
    </tr>
    <tr>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tbody>
    ...
    <tr>
      ...
      <td height="30" align="center" valign="bottom"><input type="hidden" placeholder="ลงชื่อ" class="input-data" style="width: 263px;" id="name_surname" name="name_surname" value="" ><!--(นายวรดร ไผ่เรือง)--></td>
    </tr>
    <tr>
      <td width="600">&nbsp;</td>
      <td height="30" align="center">&nbsp;</td>

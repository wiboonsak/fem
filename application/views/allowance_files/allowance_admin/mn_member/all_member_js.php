...
<!---------- table ------------------->
	...
	<script src="<?php echo base_url(); ?>assets_saraban/js/neon-demo.js"></script>
		...
		function open_modalQuota(user_id){	
			
			$("#quota").val("");			
			$("#userID").val("");
			
			$.post('<?php echo base_url('Allowance/modalQuota')?>' , { user_id : user_id }, 
			function(data){ 
						   
				var txt = data.split("+");
						   
				$('#modalQuota').modal({backdrop: 'static', keyboard: false});
				$("#modalQuota").modal();					
				$("#label_name").text(txt[0]); 
				$("#label_career").text(txt[1]);				
				$("#label_position").text(txt[2]); 
				txt[3] = txt[3].replace(/,/g, "");			   
				$("#quota").val(parseInt(txt[3]));			
				$("#userID").val(user_id);			
			})		
		}
		//-----------------------------	
		...
		function saveQuota(){
			...
			$.post('<?php echo base_url('Allowance/do_saveQuota')?>' , { user_id : user_id , quota : quota , user_update : user_update }, 
			function(data){ 
				...
				if(data != 0){
					...
					swal({
						...
						function (){
							setTimeout(function(){ window.location.href = "<?php echo base_url('Allowance/all_member')?>"; }, 1000);
						});
				} else {

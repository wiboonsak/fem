						...
						<?php } ?>							
						 ...
						 <div class="form-group" >
							  <label>3. Select Section</label>
							 	<div class="input-group">
								  <input type="text" placeholder="<?php if($papper2->section == '1'){ echo "Research Articles"; } else { echo "Academic Articles"; } ?>" class="form-control" disabled>
								</div>
						</div> 
					    <div class="form-group" >
		...
		<?php //if(($papper2->editor_changeStatus_date != '0000-00-00 00:00:00') && (($papperFile2->editor_result =='2') || ($papperFile2->editor_result =='3')) ){
          ...
          <div role="tabpanel" class="tab-pane fade <?php if($a == $round){ echo 'active in';} ?>" id="tab1-<?php echo $a?>">
              <p style="font-size: 14pt"><strong>Comment Details : Round <?php echo $a?></strong><br><br></p>
			    ...
			    <form id="1submissionFrm<?php echo $a?>" role="form" method="post" action="" enctype="multipart/form-data">
						...
						<div class="form-group" >
							...
							<div class="input-group" style="padding-left: 15px;">
								Browse File Word * <input id="file1<?php echo $a?>" type="file" placeholder="Browse File Word" class="form-control"  name="file1" onChange="check_typeFile(this.value, 'file1<?php echo $a?>', 'w')">
							</div>
						</div> 
					 	</form>
						
						<form id="2submissionFrm<?php echo $a?>" role="form" method="post" action="" enctype="multipart/form-data">
						<div class="form-group" >
							<div class="input-group" style="padding-left: 15px;">
								Browse File PDF * <input id="file2<?php echo $a?>" type="file" placeholder="Browse File PDF" class="form-control"  name="file2" onChange="check_typeFile(this.value, 'file2<?php echo $a?>', 'p')">
							</div>
							...
							<input type="hidden" id="a_remarks<?php echo $a?>" name="a_remarks" value="" >
							<input type="hidden" id="imgNameUpdate<?php echo $a?>" name="imgNameUpdate2" value="" >							
						</div> 
					 	</form>
						
						<form id="3submissionFrm<?php echo $a?>" role="form" method="post" action="" enctype="multipart/form-data">
						<div class="form-group" >
							<div class="input-group" style="padding-left: 15px;">
								Other File <input id="Other_file<?php echo $a?>" type="file" placeholder="Browse File PDF"class="form-control" name="Other_file[]" multiple >
							</div>
							<input type="hidden" id="paper_file_id<?php echo $a?>" name="paper_file_id" value="" >
						</div> 
					 	</form>
						<?php } ?>
						
						<form id="4submissionFrm<?php echo $a?>" role="form" method="post" action="" enctype="multipart/form-data">
					    <div class="form-group" >
						...
						<input type="hidden" id="editor_id<?php //echo $a?>" name="editor_id" value="<?php //echo $papper2->editor_id;?>" >-->
						 ...
						 <?php if($numEdit ==0){ ?>
							...
							<button type="button" class="button border animated middle-fill" onClick="$('#submissionFrm<?php echo $a?>')[0].reset();"><span>Cancel</span></button>
						<?php } ?>	
					  	</div>
						<?php } ?>
					</form>
...
<script>
	...
	function submission(aa){
		...
		var file1 = $('#file1'+aa).val();
		var file2 = $('#file2'+aa).val();		
		//var other_file = $('#Other_file'+aa).val();		
		var remarks = $('#remarks'+aa).val();		
		var paperID = '<?php echo $papper2->id;?>';	
		var paper_no = '<?php echo $paper_no;?>';	
		var round = '<?php echo $round;?>';	
		...
		if(file1 ==''){					
			...
			$('#a_remarks'+aa).val(remarks);
		
			var form_data = new FormData($("#1submissionFrm"+aa)[0]);
			$('#preloader').css({'display' : 'block','backgroundColor' : '#00aba617'});
			$.ajax({
				type:'POST',
				url:'<?php echo base_url('Journal/submissionEdit01')?>',
				processData: false,
				contentType: false,
				data : form_data,						  		 
	...
	function submission02(aa){		
		
			var form_data = new FormData($("#2submissionFrm"+aa)[0]);
			$('#preloader').css({'display' : 'block','backgroundColor' : '#00aba617'});
			$.ajax({
				type:'POST',
				url:'<?php echo base_url('Journal/submissionEdit02')?>',
				processData: false,
				contentType: false,
				data : form_data,						  		 
				success:function(data){
	...
	function submission03(aa){	
		...
		if(other_file != ''){
			    ...
			    var form_data = new FormData($("#3submissionFrm"+aa)[0]);
				$('#preloader').css({'display' : 'block','backgroundColor' : '#00aba617'});

				$.ajax({
					type:'POST',
					url:'<?php echo base_url('Journal/submissionEdit03')?>',
					processData: false,
					contentType: false,
					data : form_data,						  		 
					success:function(data2){

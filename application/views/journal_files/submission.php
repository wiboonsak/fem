              ...
              <p style="font-size: 11pt"><strong>Submission</strong><br><br></p>
			   ...
			   <div class="defoult-form form-2">
			  	 	...
			  	 	<div class="form-group" >
						  <label>2. Upload File</label>
							<div class="input-group" style="padding-left: 15px;">								
							  ...
							  File Word * <input id="file1" type="file" placeholder="Browse File Word" class="form-control" name="file1" onChange="check_typeFile(this.value, 'file1', 'w')">
								
								<input type="hidden" id="title2" name="title2" value="" >
								<input type="hidden" id="section2" name="section2" value="" >
								<input type="hidden" id="abstract2" name="abstract2" value="" >
								<input type="hidden" id="remarks2" name="remarks2" value="" >				
				 ...
				 <form id="frm2" role="form" method="post" action="" enctype="multipart/form-data">	
				   ...
				   <div class="form-group">						  
							<div class="input-group" style="padding-left: 15px;">					
							  File PDF * <input id="file2" type="file" placeholder="Browse File PDF" class="form-control" name="file2" onChange="check_typeFile(this.value, 'file2', 'p')">
								
								<input type="hidden" id="imgNameUpdate2" name="imgNameUpdate2" value="" >
								<input type="hidden" id="paperID" name="paperID" value="" >
								<input type="hidden" id="remarks3" name="remarks3" value="" >					
							</div>
					 </div> 
				</form>	
				...
				<form id="frm3" role="form" method="post" action="" enctype="multipart/form-data">	
				   ...
				   <div class="form-group" >						  
							<div class="input-group" style="padding-left: 15px;">								
								...
								<input type="hidden" id="paper_file_id" name="paper_file_id" value="" >			
							</div>
					 </div> 
				</form>	
...
//------------------------------------------
	...
	/*function submission(){
		...
		if(title ==''){				
			...
			$('#remarks3').val(remarks);
			$('#section2').val(section);
			$('#abstract2').val(abstract);		
		
			var form_data = new FormData($("#frm1")[0]);
			//var form_data2 = new FormData($("#frm2")[0]);
			//var form_data3 = new FormData($("#frm3")[0]);
			$('#preloader').css({'display' : 'block','backgroundColor' : '#00aba617'});
			
			$.ajax({
				type:'POST',
				url:'<?php //echo base_url('Journal/submissionPaper01')?>',
				processData: false,
				contentType: false,
				data : form_data,						  		 
				success:function(data){
					...
					//var imgName = myObj.imgName;
					//var paperID = myObj.paperID;					
					$('#paperID').val(myObj.paperID);
					$('#imgNameUpdate2').val(myObj.imgName);					
			    }
            }).then(function(data){	
				...
				$.ajax({
				type:'POST',
				url:'<?php //echo base_url('Journal/submissionPaper02')?>',
				processData: false,
				contentType: false,
				data : form_data2,						  		 
				success:function(data2){
					
					$('#paper_file_id').val(data2);	
					console.log('2pp.....'+$('#paper_file_id').val());									
					console.log('2.....'+data2);									
			    }
            })	
				...
				if(other_file != ''){
					...
					var form_data3 = new FormData($("#frm3")[0]);
				   		...
				   		$.ajax({
						type:'POST',
						url:'<?php //echo base_url('Journal/submissionPaper03')?>',
						processData: false,
						contentType: false,
						data : form_data3,						  		 
						success:function(data3){
							...
							if(data3 == 1){
								...
								$.post('<?php //echo base_url('Journal_Mail/mail_authorSubmission')?>' , { dataID : paperID }, function(data){
...
//----------------------------------------	
	...
	function testMulti(){
			...
			var form_data = new FormData($("#test1")[0]);
			var form_data2 = new FormData($("#test2")[0]);
			var form_data3 = new FormData($("#test3")[0]);
			$('#preloader').css({'display' : 'block','backgroundColor' : '#00aba617'});
			
			$.ajax({
				type:'POST',
				url:'<?php echo base_url('Journal/testFN')?>',
				processData: false,
				contentType: false,
				data : form_data,						  		 
				success:function(data){
				...
				$.ajax({
				type:'POST',
				url:'<?php echo base_url('Journal/testFN2')?>',
				processData: false,
				contentType: false,
				data : form_data2,						  		 
				success:function(data2){
				...
				$.ajax({
				type:'POST',
				url:'<?php echo base_url('Journal/testFN3')?>',
				processData: false,
				contentType: false,
				data : form_data3,						  		 
				success:function(data3){

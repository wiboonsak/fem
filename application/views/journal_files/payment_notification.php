...
<section class="inner-intro bg bg-fixed bg-overlay-black-70" style="background-image:url(<?php echo base_url('journal-html/images/bg/bg-2.jpg')?>">	
  ...
  <div class="container">
     <div class="row intro-title text-center">
           <div class="col-sm-12">
				<div class="section-title"><h1 class="title text-white">Payment</h1></div>
           </div>
           <div class="col-sm-12">
					...
					<form id="paymentFrm" role="form" method="post" action="" enctype="multipart/form-data">
                     ...
                     <?php	$paymentData = $this->journal_model_2->list_paymentData($payment2->paper_no); 
							...
							if($paymentDatanum > 0){
								...
								$txt2 = number_format($paymentData2->Amount);
								$txt3 = $this->journal_model->GetEngDate($paymentData2->Date);
								$time = $paymentData2->Time;
								$Note = $paymentData2->Note;								
							}							
					 ?>
                                            
					 <div class="form-group">
						  	<label>Amount (THB Baht)&nbsp;&nbsp;<?php echo $txt2?></label>
						 	<?php if($txt2 == ''){ ?>
							<div class="input-group"><input id="Amount" type="number" class="form-control"  name="Amount"></div>
						 	<?php } ?>
					 </div>
                                            
					 <div class="form-group">
							<label>Date&nbsp;&nbsp;<?php echo $txt3?></label>
						 	<?php if($txt3 == ''){ ?>
						 	<div class="input-group"><input type="text" class="form-control" name="Date" id="Date" data-date-format="dd/mm/yyyy" ></div>	
						 	<?php } ?>
					 </div>
			  	 	 ...
			  	 	 <div class="form-group" >
                            ...
                            <?php if($paymentDatanum == 0){ ?>
                                ...
                                <?php if($paymentDatanum > 0){   
										...
										$fileArray = explode(".",$paymentData2->Slip);							
										$allowed = array('gif','jpg','png','jpeg','GIF','JPG','PNG','JPEG');
										$filename = $fileArray[1];
										if(!in_array($filename,$allowed)){  ?>
									
											<a href="<?php echo base_url().'uploadfile/'.$paymentData2->Slip?>" target="_blank" ><label><?php echo $paymentData2->Slip?></label></a>
											
								<?php	} else { ?>
											
											<img src="<?php echo base_url().'uploadfile/'.$paymentData2->Slip?>" alt="image" width="200" height="225" />
								<?php }  } ?>
                                </div>							 
							</div>
					 </div>				  	
					 ...
					 <div class="form-group" >
							<label>Note&nbsp;&nbsp;<?php echo $Note?></label>
						 	<?php if($time == ''){ ?>						 

...
if(in_array($userDetail2->career_id, $array1)){
	...
	if($documentData2->vacation == '0'){
        ...
        $txt_vacation = '';$txt_vacation2='';
        $numvacation = $vacationData->num_rows();
        foreach($vacationData->result() as $vacationData2){}
         
        if(($documentData2->vacation == '1')&&($documentData2->all_vacation == '1')){
            
            $txt_vacation = ' (ลาพักผ่อนวันที่ '.$vacationData2->date_vacation.')';
        }
        if(($documentData2->vacation == '1')&&($documentData2->all_vacation == '0')){
           
            if($numvacation==1){
                $txt_vacation = '('.$vacationData2->name.' ลาพักผ่อนวันที่ '.$vacationData2->date_vacation.')';
            }else if($numvacation>1){
                $txt_vacation2 = '<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tbody><tr>
      <td align="left">หมายเหตุการลาพักร้อน<br>';
                $n = 1;
      foreach($vacationData->result() as $vacationData3){
      $txt_vacation2 = $txt_vacation2.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$n.'.&nbsp;'.$vacationData3->name.'&nbsp; ลาพักร้อนวันที่ &nbsp;'.$vacationData3->date_vacation.'.<br>';$n++;}
      $txt_vacation2 = $txt_vacation2.'</td>
      </tr>
      </tbody>
      </table>';
            }else{
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tbody>
    <tr>      
      <td align="left"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;จึงเรียนมาเพื่อโปรดพิจารณาด้วย จะเป็นพระคุณยิ่ง</td>
    </tr>
  </tbody>
...
if($documentData2->approve_id != 0){
  ...
  <tbody>
    <tr>';
        ...
        $getsetadminsystem = $this->Allowance_model->getsetadminsystem('1','1,2');
        foreach ($getsetadminsystem->result() AS $getsetadminsystem2){}
         if($getsetadminsystem2->titlename!=''){
            $adminname = $getsetadminsystem2->titlename.$getsetadminsystem2->firstname.' '.$getsetadminsystem2->lastname;
        }else{
            $adminname = $getsetadminsystem2->firstname.' '.$getsetadminsystem2->lastname; 
        }
     $html = $html.' <td width="40%" align="left" style="border: 1px solid black;font-size:16px ">
...
//                }
//		
//	$html = $html.'<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
//  <tbody>';
//  

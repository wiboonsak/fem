...
//				}
    ...
    function calculate_totalPrice(amount,type){
		...
		if((totalPoints > balance) && (reason_request == '1')){ 
		    ...
		    alert('ตามสิทธิคงเหลือ '+balance.toLocaleString()+' บาท กรุณาตรวจสอบและแก้ไขข้อมูล !');
		    $('#btn1').attr("disabled", true);
		    $('#chkrules').attr("disabled", true);
		    $('label.cb-wrapper').css('cursor', 'not-allowed');
                    
		}else{
                    $('#chkrules').attr("disabled", false);
		    $('label.cb-wrapper').css('cursor', 'pointer');
                }	
...
//--------------------------------
function savedata(idsaraban,table,type_travel){
     var type_travel1 = $('#type_travel1').val();
     $.post('<?php echo base_url('Inputform/getdocfile')?>' , { idsaraban : idsaraban,type_travel:type_travel}, 
			function(data1){
                            if(data1!=0){

 ...
 { 
	...
	function list_paperData($userID=NULL, $status1=NULL){	
		...
		if($userID ==''){
			...
			$sql = "SELECT * FROM `tbl_papers_data` WHERE archive != '1' AND status_process IN(".$status1.") ORDER BY id DESC ";
			$query=$this->db->query($sql);	
			return $query;	
		} else {
		...
		/*	$txtWhere2 = "AND editor_id ='".$userID."' ";
			...
			//	SELECT * FROM `tbl_papers_data` WHERE status_process IN('1') AND editor_id ='2' OR id IN('4')
				...
				$sql1 = "SELECT paper_id FROM `tbl_reviewer_data` WHERE reviewer_id = '".$userID."' AND edit_result != '4' AND status IN('0','1') ";
			...
			//SELECT p.* FROM tbl_papers_data AS p JOIN tbl_paper_file AS f ON p.id = f.paper_id WHERE f.editor_result != '4' AND  p.status_process IN('1') AND p.editor_id = '18'  ORDER BY p.id DESC
			
				//$sql = "SELECT * FROM `tbl_papers_data` WHERE status_process IN(".$status1.") AND editor_id = '".$userID."' $txt2 ORDER BY id DESC ";
			
				$sql = "SELECT p.* FROM tbl_papers_data AS p JOIN tbl_paper_file AS f ON p.id = f.paper_id WHERE f.editor_result != '4' AND p.status_process IN(".$status1.") AND p.editor_id = '".$userID."' $txt2 GROUP BY p.id ORDER BY p.id DESC ";
				$query=$this->db->query($sql);	
				return $query;	
			//}			
		}		
	...
	//---------------------------  get_reviewerList($paper_id,$round)
	function get_reviewerList3($paper_id=NULL,$round_number=NULL,$check=NULL){
	...
	function list_paperAccepted($userID=NULL,$status1=NULL,$author_id=NULL){
		...
		if($txt !=''){
			...
			if((($this->session->userdata('juserLv') =='1') || ($this->session->userdata('juserLv') =='2')) && ($status1 == '4')){
			
				$txtWhere3 = "OR status_process = '3'";
			}
			if((($this->session->userdata('juserLv') =='1') || ($this->session->userdata('juserLv') =='2')) && ($status1 == '1')){
			
				$txtWhere3 = "OR status_process = '4'";
			}			
		} else {
			$txt2 ='';
			...
			if((($this->session->userdata('juserLv') =='1') || ($this->session->userdata('juserLv') =='2')) && ($status1 == '4')){
				...
				$txtWhere3 = "status_process = '3'";
			}
			if((($this->session->userdata('juserLv') =='1') || ($this->session->userdata('juserLv') =='2')) && ($status1 == '1')){
			
				$txtWhere3 = "status_process = '4'";
			}		
		}		
		if($numberCount >0){
			...
			if((($this->session->userdata('juserLv') =='1') || ($this->session->userdata('juserLv') =='2')) && ($status1 == '4')){
				...
				$sql = "SELECT * FROM `tbl_papers_data` WHERE status_process = '3' ORDER BY id DESC ";
				$query=$this->db->query($sql);	
				return $query;
				
			} else if((($this->session->userdata('juserLv') =='1') || ($this->session->userdata('juserLv') =='2')) && ($status1 == '1')){
				
				$sql = "SELECT * FROM `tbl_papers_data` WHERE status_process = '4' ORDER BY id DESC ";
				$query=$this->db->query($sql);	
				return $query;
	...
	function GetEngDateTime2($day){
       ...
       //$monthArray=Array("01"=>"January","02"=>"February","03"=>"March","04"=>"April","05"=>"May","06"=>"June","07"=>"July","08"=>"August","09"=>"September","10"=>"October","11"=>"November","12"=>"December");
		if($date < 10){ $date = str_replace("0", "", $date); } 
		return $date."/".$mon."/".$year." ".$DateTimeArray[1];
	} 
	//---------------------------  
	...
	//---------------------------  
	function searchPaper33($dateStart=NULL,$dateEnd=NULL,$status=NULL){ 
		$txtWhere2 ='';
	...
	function searchPaper($dateStart=NULL,$dateEnd=NULL,$status=NULL){ 
		...
		//SELECT * FROM tbl_papers_data AS p JOIN tbl_paper_file AS f ON p.id = f.paper_id

//WHERE p.status_process IN('0', '1', '2', '3', '4', '5', '6', '7') OR p.id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1'  AND review_date BETWEEN '2018-11-19' AND '2018-11-30' GROUP BY paper_id )
		...
		/*} else {
			...
			$sql = "SELECT * FROM tbl_papers_data WHERE status_process != '0' OR id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' GROUP BY paper_id )";			
			
		//}
		
		//if(($dateStart != '0000-00-00') && ($dateEnd != '0000-00-00')){
		} else if(($dateStart != '') && ($dateEnd != '') && ($status == '')){
		...
		/*} else {
			...
			$sql = "SELECT * FROM tbl_papers_data WHERE status_process != '0' AND date_add BETWEEN '".$dateStart."' AND '".$dateEnd."' OR id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' AND review_date BETWEEN '".$dateStart."' AND '".$dateEnd."' GROUP BY paper_id )";
		//}
		...
		//}
			...
			$sql = "SELECT * FROM tbl_papers_data WHERE status_process != '0' AND date_add = '".$dateStart."' OR id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' AND review_date = '".$dateStart."' GROUP BY paper_id )";
		...
		/*} else {
			...
			$sql = "SELECT * FROM tbl_papers_data WHERE status_process != '0' AND date_add <= '".$dateEnd."' OR id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' AND review_date <= '".$dateEnd."' GROUP BY paper_id )";
		...
		/*} else {
			...
			if($status == '5'){
				...
				$sql = "SELECT * FROM tbl_papers_data WHERE status_process = '1' AND date_add BETWEEN '".$dateStart."' AND '".$dateEnd."' OR id IN(SELECT paper_id FROM `tbl_paper_file` WHERE editor_result = '5' AND date_add BETWEEN '".$dateStart."' AND '".$dateEnd."' )";
				
			} else if($status == '4'){
				
				$sql = "SELECT * FROM tbl_papers_data WHERE status_process = '3' AND date_add BETWEEN '".$dateStart."' AND '".$dateEnd."' OR id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' AND result_status = '4' AND review_date BETWEEN '".$dateStart."' AND '".$dateEnd."' GROUP BY paper_id )";
				
			} else if($status == '1'){
				
				$sql = "SELECT * FROM tbl_papers_data WHERE status_process = '4' AND date_add BETWEEN '".$dateStart."' AND '".$dateEnd."' OR id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' AND result_status = '1' AND review_date BETWEEN '".$dateStart."' AND '".$dateEnd."' GROUP BY paper_id )";
				
			} else {
			
				$sql = "SELECT * FROM tbl_papers_data WHERE id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' AND result_status = '".$status."' AND review_date BETWEEN '".$dateStart."' AND '".$dateEnd."' GROUP BY paper_id )";			
			}
		//}
		...
		//if(($dateStart != '0000-00-00') && ($dateEnd != '0000-00-00')){
			...
			if($status == '5'){
				...
				$sql = "SELECT * FROM tbl_papers_data WHERE status_process = '1' OR id IN(SELECT paper_id FROM `tbl_paper_file` WHERE editor_result = '5' )";
				
			} else if($status == '4'){
				
				$sql = "SELECT * FROM tbl_papers_data WHERE status_process = '3' OR id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' AND result_status = '4' GROUP BY paper_id )";
				
			} else if($status == '1'){
				
				$sql = "SELECT * FROM tbl_papers_data WHERE status_process = '4' OR id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' AND result_status = '1' GROUP BY paper_id )";
				
			} else {
			
				$sql = "SELECT * FROM tbl_papers_data WHERE id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' AND result_status = '".$status."' GROUP BY paper_id )";			
			}
		...
		/*} else {
			...
			if($status == '5'){
				...
				$sql = "SELECT * FROM tbl_papers_data WHERE status_process = '1' AND date_add = '".$dateStart."' OR id IN(SELECT paper_id FROM `tbl_paper_file` WHERE editor_result = '5' AND date_add = '".$dateStart."' )";
				
			} else if($status == '4'){
				
				$sql = "SELECT * FROM tbl_papers_data WHERE status_process = '3' AND date_add = '".$dateStart."' OR id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' AND result_status = '4' AND review_date = '".$dateStart."' GROUP BY paper_id )";
				
			} else if($status == '1'){
				
				$sql = "SELECT * FROM tbl_papers_data WHERE status_process = '4' AND date_add = '".$dateStart."' OR id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' AND result_status = '1' AND review_date = '".$dateStart."' GROUP BY paper_id )";
				
			} else {
			
				$sql = "SELECT * FROM tbl_papers_data WHERE id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' AND result_status = '".$status."' AND review_date = '".$dateStart."' GROUP BY paper_id )";			
			}
		//}
		...
		/*} else {
			...
			if($status == '5'){
				...
				$sql = "SELECT * FROM tbl_papers_data WHERE status_process = '1' AND date_add <= '".$dateEnd."' OR id IN(SELECT paper_id FROM `tbl_paper_file` WHERE editor_result = '5' AND date_add <= '".$dateEnd."' )";
				
			} else if($status == '4'){
				
				$sql = "SELECT * FROM tbl_papers_data WHERE status_process = '3' AND date_add <= '".$dateEnd."' OR id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' AND result_status = '4' AND review_date <= '".$dateEnd."' GROUP BY paper_id )";
				
			} else if($status == '1'){
				
				$sql = "SELECT * FROM tbl_papers_data WHERE status_process = '4' AND date_add <= '".$dateEnd."' OR id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' AND result_status = '1' AND review_date <= '".$dateEnd."' GROUP BY paper_id )";
				
			} else {
			
				$sql = "SELECT * FROM tbl_papers_data WHERE id IN(SELECT paper_id FROM `tbl_review_comment` WHERE user_type = '1' AND result_status = '".$status."' AND review_date <= '".$dateEnd."' GROUP BY paper_id )";			
			}
		//}
		...
		//$sql = "SELECT * FROM `tbl_review_comment` WHERE user_type2 = '1' $txtWhere2  $txtWhere ORDER BY review_date DESC  ";
		$query=$this->db->query($sql);	
		return $query;	
	}  
	//--------------------------- 
	...
	//---------------------------  
	function numberPaper_data($dateStart=NULL,$dateEnd=NULL){ 
		...
		if(($dateStart == '') && ($dateEnd == '')){
			...
			$dateEnd = date('Y-m-d',strtotime($dateEnd . "+1 days"));
			$txtWhere2 = "AND f.date_add BETWEEN '".$dateStart."' AND '".$dateEnd."' ";
			...
			$txtWhere2 = "AND f.date_add >= '".$dateStart."' ";
			...
			$dateEnd = date('Y-m-d',strtotime($dateEnd . "+1 days"));
			$txtWhere2 = "AND f.date_add <= '".$dateEnd."' ";
		
		}
		$sql = "SELECT p.paper_no, p.title, p.author_id, f.date_add AS f_date_add, f.editor_id, f.editor_result, f.round_number, p.date_add AS p_date_add FROM `tbl_paper_file` AS f LEFT JOIN tbl_papers_data AS p ON f.paper_id = p.id WHERE f.paper_id = p.id $txtWhere2 ORDER BY f.date_add DESC ";
		$query=$this->db->query($sql);	
		return $query;	
	}  
	//----------------------------	 
	function list_authorID($name=NULL){ 
		
		$sql1 = "SELECT * FROM `tbl_authors_data` WHERE `first_name` LIKE '%".$name."%' OR `last_name` LIKE '%".$name."%' ";
        $query1 = $this->db->query($sql1);			
		$numberCount = $query1->num_rows();	
		$txt ='';
		if($numberCount >0){
	...
	//----------------------------	 
	function paper_byAuthor($authorID=NULL){
		
		if($authorID !=''){
			$txtWhere2 = " AND p.author_id IN($authorID) ";
		} else {
			$txtWhere2 = "";
		}		
		$sql = "SELECT p.paper_no, p.title, p.author_id, f.date_add AS f_date_add, f.editor_id, f.editor_result, f.round_number, p.date_add AS p_date_add FROM `tbl_paper_file` AS f LEFT JOIN tbl_papers_data AS p ON f.paper_id = p.id WHERE f.paper_id = p.id $txtWhere2 ORDER BY f.date_add DESC ";
	...
	//---------------------------  
	function listemaildetail2($dataID=NULL, $paper_no=NULL, $typeMail=NULL){				
			
		$this->db->where('type_mail', $typeMail);						
		$this->db->where('paper_no', $paper_no);						
		$this->db->where('user_id', $dataID);						
		$this->db->select('*');		
		$query = $this->db->get('tbl_mail_data');		
		return $query;		
	} 	
	//---------------------------	 
	function update_sendmailReviewer($dataID=NULL,$reviewer_id=NULL,$paper_id=NULL){			 

...
//------------------------------
    ...
    //---------------------------	 
    function get_listSection($currentID) {
     ...
     function getCategory($datatype){
         ...
         //------------------------------
          function geteditList(){
              $sql=$this->db->query("SELECT a.* ,  b.category_title FROM tbl_journal_editorial a LEFT JOIN tbl_journal_editorial_cate b ON a.editorial_category=b.id WHERE a.show_onweb='1' AND b.show_onweb='1' ORDER BY b.id ASC , b.id ASC ");
		 return $sql;    
	 }
...
if ($this->db->insert('tbl_journal_PDF', $data)) {
   ...
   function userdetail() {
        ...
        $sql = $this->db->query("SELECT * FROM tbl_journal_user WHERE  admin_type !='1' AND data_status = '1' AND id != '".$userID."' ORDER BY admin_type ASC ");
        return $sql;
   }
   //-------------------------------------

...
?>
		...
		<tbody>
			...
			<tr class="tr-bg">
				...
				<?php } else if(($txt == '0') && ($paper2->status_process == '4') && ($paper2->editor_id == 0)){ ?>
					<button type="button" class="btn btn-success" style="color: #FFFFFF">Accepted</button>
...
//////////////////////////		
    ...
    //-------------------
	public function submissionEdit01(){
	...
	//-------------------
	public function submissionEdit02(){ 		
		
		$author_id = $this->session->userdata('jlogin');				
		$remarks = $this->input->post('a_remarks');		
		$paperID = $this->input->post('paper_id');		
		$round = $this->input->post('round');	
		$editor_id = $this->input->post('editor_id');	
		$imgNameUpdate2 = $this->input->post('imgNameUpdate2');	
	...
	//-------------------
	public function submissionEdit03(){ 		
		...
		if($countFiles >0){ 
			...
			for($i=0; $i<$countFiles; $i++){           
                ...
                if($this->upload->do_upload('file_upload2')){
                    $fileData = $this->upload->data();
                    $uploadData[$i]['file_name'] = $fileData['file_name'];
					$Result3 = $this->journal_model->add_otherFiles($paper_file_id,$uploadData[$i]['file_name']);
                }				
			}
			$Result = 1;
		}				
...
<?php 	}	 } 
	...
	public function testFN3(){
		...
		//////////////////////////	
		$countFiles = count($_FILES['f33']['name']); 		
		if($countFiles >0){ 
			$xx = $countFiles;
		}
		echo $xx;
	...
	public function submissionPaper01(){
		//$author_id = $this->input->post('author_id');	
		$author_id = $this->session->userdata('jlogin');
		$title = $this->input->post('title2');		
		$remarks = $this->input->post('remarks2');		
		$section = $this->input->post('section2');		
		$abstract = $this->input->post('abstract2');	
...
//////////////////////////		
		...
		if($_FILES['file1']['name'] !=''){
				...
				if(!$this->upload->do_upload('file1')){
                   ...
                   // $this->handle_error($this->upload->display_errors());
                    //$is_file_error = TRUE;
					  echo "ErrorUpload1:".$this->upload->display_errors();
                } else { 
				  	$image_data = $this->upload->data();
                    //$config['image_library'] = 'gd2';
                    $config['source_image'] = $image_data['full_path']; //get original image
		...
		if($imgNameUpdate2 != 'Null'){
			
			$paperID = $this->journal_model->insertPaper($author_id, $title, $remarks, $section, $abstract);
		}
		
		$data['imgName'] = $imgNameUpdate2;
	...
	public function submissionPaper02(){
...
//////////////////////////		
	...
	public function submissionPaper03(){
	...
	/////////////////////////////	
		...
		//echo $Result.'...paper_file_id->'.$paper_file_id;	
		echo $Result;	
	}
	//-------------------	
